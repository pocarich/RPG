...
void BattleEffectE003::Update()
{
	...
	if (effectTimer == 1)
	{
		...
		{
			playerMap->CauseByEnemy(enemyStatus,direction, sourceID, 3);
		}
	}
	if (effectTimer == 15)
	{
...
void BattleEffectE003::Draw()
{
	DrawRotaGraphF((float)position.x - camera.x, (float)position.y - camera.y, 1.0, 0.0, graph[28][(effectTimer / 3) % 5], TRUE);
}