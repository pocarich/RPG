...
BattleEffectE009::~BattleEffectE009()
{

}

void BattleEffectE009::Update()
{
	effectTimer++;
	if (effectTimer == 1)
	{
		...
		{
			playerMap->CauseByEnemy(enemyStatus,direction, sourceID, 9);
		}
	}
	if (effectTimer == 30)
	{
...
void BattleEffectE009::Draw()
{
	SetDrawBlendMode(DX_BLENDMODE_ADD, 255);
	DrawRotaGraphF((float)position.x - camera.x, (float)position.y - camera.y, 1.0, 0.0, graph[32][(effectTimer / 3) % 10], TRUE);
