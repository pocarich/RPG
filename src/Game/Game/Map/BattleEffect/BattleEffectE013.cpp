...
void BattleEffectE013::Update()
{
	...
	if (effectTimer == 1)
	{
		...
		{
			playerMap->CauseByEnemy(enemyStatus,direction, sourceID, 13);
		}
	}
	if (effectTimer == 24)
	{
...
void BattleEffectE013::Draw()
{
	DrawRotaGraphF((float)position.x - camera.x, (float)position.y - camera.y, 1.0, 0.0, graph[36][(effectTimer / 3) % 8], TRUE);
}