...
void BattleEffectE122::Update()
{
	...
	if (effectTimer == 1)
	{
		...
		{
			playerMap->CauseByBoss(bossStatus, direction, sourceID, 122);
		}
	}
	...
	{
		{
			...
			if (CanMoveToNextPosition(next, direction))
			{
				...
				battleEffectList.push_back(make_shared<BattleEffectE122>(bossStatus, playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, next, direction, camera, mapNum, stateCount + 1, newOffset));
			}
		}
		...
		if (width[stateCount + 1] > width[stateCount])
		{
			if (offset == 0)
			{
				...
				if (CanMoveToNextPosition(next, direction))
				{
					battleEffectList.push_back(make_shared<BattleEffectE122>(bossStatus, playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, next, direction, camera, mapNum, stateCount + 1, 0));
				}
			}
			if (offset == 2 * width[stateCount])
			{
				...
				if (CanMoveToNextPosition(next, direction))
				{
					battleEffectList.push_back(make_shared<BattleEffectE122>(bossStatus, playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, next, direction, camera, mapNum, stateCount + 1, offset + 2));
				}
			}
		}
	}
