...
void BattleEffectE015::Update()
{
	...
	if (effectTimer == 1)
	{
		...
		{
			playerMap->CauseByEnemy(enemyStatus,direction, sourceID, 15);
		}
	}
	if (effectTimer == 30)
	{
...
void BattleEffectE015::Draw()
{
	DrawRotaGraphF((float)position.x - camera.x, (float)position.y - camera.y, 1.0, 0.0, graph[37][(effectTimer / 3) % 10], TRUE);
}