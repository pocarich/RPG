...
BattleEffectE116::~BattleEffectE116()
{

}

void BattleEffectE116::Update()
{
	static const int width[] = { 0,1,2,3,4,5,5,6,6 };
	effectTimer++;
	if (effectTimer == 1)
	{
	...
	{
		{
			...
			if (CanMoveToNextPosition(next, direction))
			{
				...
				battleEffectList.push_back(make_shared<BattleEffectE116>(bossStatus, playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, next, direction, camera, mapNum, stateCount + 1, newOffset));
			}
		}
		...
		if (width[stateCount + 1] > width[stateCount])
		{
			if (offset == 0)
			{
				...
				if (CanMoveToNextPosition(next, direction))
				{
					battleEffectList.push_back(make_shared<BattleEffectE116>(bossStatus, playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, next, direction, camera, mapNum, stateCount + 1, 0));
				}
			}
			if (offset == 2 * width[stateCount])
			{
				...
				if (CanMoveToNextPosition(next, direction))
				{
					battleEffectList.push_back(make_shared<BattleEffectE116>(bossStatus, playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, next, direction, camera, mapNum, stateCount + 1, offset + 2));
				}
			}
		}
	}
