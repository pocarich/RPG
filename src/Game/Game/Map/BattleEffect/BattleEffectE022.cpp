...
BattleEffectE022::BattleEffectE022(EnemyStatus enemyStatus, PlayerMap* playerMap, list<BossMapPtr>& bossMapList, list<ObjectMapPtr>& objectMapList, list<BattleEffectPtr>& battleEffectList, list<EnemyMapPtr>& enemyMapList, int sourceID, Vector2<int> masuPosition, Direction direction, Vector2<double>&camera, int& mapNum) :BattleEffect(playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, ObjectType::PLAYER, sourceID, masuPosition, direction, camera, mapNum, TargetType::PLAYER),enemyStatus(enemyStatus)
{
	switch (direction)
	{
	case Direction::DOWN:
...
BattleEffectE022::~BattleEffectE022()
{

}

void BattleEffectE022::Update()
{
	effectTimer++;
	if (effectTimer %interval == 1 && effectTimer <= (interval * 2 + 1))
	{
		...
		{
			playerMap->CauseByEnemy(enemyStatus,direction, sourceID, 22);
		}
	}
