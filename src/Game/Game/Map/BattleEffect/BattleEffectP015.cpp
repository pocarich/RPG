...
BattleEffectP015::BattleEffectP015(PlayerMap* playerMap, list<BossMapPtr>& bossMapList, list<ObjectMapPtr>& objectMapList, list<BattleEffectPtr>& battleEffectList, list<EnemyMapPtr>& enemyMapList, int sourceID, Vector2<int> masuPosition, Direction direction, Vector2<double>&camera, int& mapNum,int count, bool SPMAX) :BattleEffect(playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, ObjectType::PLAYER, sourceID, masuPosition, direction, camera, mapNum, TargetType::ENEMY),count(count), SPMAX(SPMAX)
{
	switch (direction)
	{
	case Direction::DOWN:
...
BattleEffectP015::~BattleEffectP015()
{

}

void BattleEffectP015::Update()
{
	effectTimer++;
	if (effectTimer == 1)
	{
		for (auto& obj : enemyMapList)
		{
			...
			{
				obj->CauseByPlayer(sourceID, 15,SPMAX);
			}
		}
	...
	if (effectTimer == createNextEffectTime&&count != effectMax)
	{
		...
		if (canCreate)
		{
			battleEffectList.push_back(make_shared<BattleEffectP015>(playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, masuPosition, direction, camera, mapNum, count + 1,SPMAX));
		}
	}
...
void BattleEffectP015::Draw()
{
	DrawRotaGraphF((float)position.x - camera.x, (float)position.y - 10 -camera.y, 1.0, 0.0, graph[21][(effectTimer / 3) % 8], TRUE);
}