...
void BattleEffectE071::Update()
{
	...
	if (effectTimer == 1)
	{
		for (auto& pos : hitPositionList)
		{
			...
			{
				playerMap->CauseByEnemy(enemyStatus, direction, sourceID, 71);
			}
		}
	}
	if (effectTimer == 30)
...
void BattleEffectE071::Draw()
{
	DrawRotaGraphF((float)position.x - camera.x, (float)position.y - camera.y, 1.0, 0.0, graph[55][(effectTimer / 2) % 15], TRUE);
}