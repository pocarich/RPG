...
BattleEffectE113::BattleEffectE113(BossStatus bossStatus, PlayerMap* playerMap, list<BossMapPtr>& bossMapList, list<ObjectMapPtr>& objectMapList, list<BattleEffectPtr>& battleEffectList, list<EnemyMapPtr>& enemyMapList, int sourceID, Vector2<int> masuPosition, Direction direction, Vector2<double>&camera, int& mapNum, int value1, int value2) :BattleEffect(playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, ObjectType::PLAYER, sourceID, masuPosition, direction, camera, mapNum, TargetType::PLAYER), bossStatus(bossStatus), value1(value1), value2(value2)
{
	static const Vector2<int> offset1[] = { Vector2<int>(4,0),Vector2<int>(3,-3),Vector2<int>(0,-4),Vector2<int>(-3,-3) };
	static const Vector2<int> offset2[] = { Vector2<int>(-4,0),Vector2<int>(-3,3),Vector2<int>(0,4),Vector2<int>(3,3) };
...
BattleEffectE113::~BattleEffectE113()
{

}

void BattleEffectE113::Update()
{
	effectTimer++;
	if (effectTimer == 1)
	{
		for (auto& pos : hitPositionList)
		{
			...
			{
				playerMap->CauseByBoss(bossStatus, direction, sourceID, 113);
			}
		}
	}
	if (effectTimer == 15)
	{
		if (value2 < 3)
		{
			battleEffectList.push_back(make_shared<BattleEffectE113>(bossStatus, playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, masuPosition, direction, camera, mapNum, value1, value2 + 1));
		}
	}
	if (effectTimer == 30)
	{
