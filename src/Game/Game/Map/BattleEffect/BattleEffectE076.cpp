...
BattleEffectE076::BattleEffectE076(EnemyStatus enemyStatus, PlayerMap* playerMap, list<BossMapPtr>& bossMapList, list<ObjectMapPtr>& objectMapList, list<BattleEffectPtr>& battleEffectList, list<EnemyMapPtr>& enemyMapList, int sourceID, Vector2<int> masuPosition, Direction direction, Vector2<double>&camera, int& mapNum) :BattleEffect(playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, ObjectType::PLAYER, sourceID, masuPosition, direction, camera, mapNum, TargetType::PLAYER), enemyStatus(enemyStatus)
{
...
BattleEffectE076::~BattleEffectE076()
{

}

void BattleEffectE076::Update()
{
	effectTimer++;
	if (effectTimer == 1)
	{
		for (auto& pos : attackPositionList)
		{
			...
			{
				playerMap->CauseByEnemy(enemyStatus, direction, sourceID, 76);
			}
		}
	}
	if (effectTimer == 30)
...
void BattleEffectE076::Draw()
{
	DrawRotaGraphF((float)position.x - camera.x, (float)position.y - camera.y, 1.0, 0.0, graph[59][(effectTimer / 3) % 10], TRUE);
}