...
BattleEffectE058::BattleEffectE058(EnemyStatus enemyStatus, PlayerMap* playerMap, list<BossMapPtr>& bossMapList, list<ObjectMapPtr>& objectMapList, list<BattleEffectPtr>& battleEffectList, list<EnemyMapPtr>& enemyMapList, int sourceID, Vector2<int> masuPosition, Direction direction, Vector2<double>&camera, int& mapNum,int count) :BattleEffect(playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, ObjectType::PLAYER, sourceID, masuPosition, direction, camera, mapNum, TargetType::PLAYER), enemyStatus(enemyStatus),count(count)
{
	if (count == 0)
	{
		switch (direction)
...
BattleEffectE058::~BattleEffectE058()
{

}

void BattleEffectE058::Update()
{
	effectTimer++;
	if (effectTimer == 1)
	{
		for (auto& pos : hitPositionList)
		{
			...
			{
				playerMap->CauseByEnemy(enemyStatus, direction, sourceID, 58);
			}
		}
	}
	...
	if (effectTimer == popNextTime&&count + 1 < effectMax)
	{
		...
		{
			battleEffectList.push_back(make_shared<BattleEffectE058>(enemyStatus, playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, masuPosition, direction, camera, mapNum, count + 1));
		}
	}
	if (effectTimer == 21)
	{
