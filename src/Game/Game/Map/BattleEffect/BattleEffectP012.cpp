...
const int BattleEffectP012::distance = 5;

BattleEffectP012::BattleEffectP012(PlayerMap* playerMap, list<BossMapPtr>& bossMapList, list<ObjectMapPtr>& objectMapList, list<BattleEffectPtr>& battleEffectList, list<EnemyMapPtr>& enemyMapList, int sourceID, Vector2<int> masuPosition, Direction direction, Vector2<double>&camera, int& mapNum,bool SPMAX) :BattleEffect(playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, ObjectType::PLAYER, sourceID, masuPosition, direction, camera, mapNum, TargetType::ENEMY),SPMAX(SPMAX)
{
	switch (direction)
	{
	case Direction::DOWN:
...
BattleEffectP012::~BattleEffectP012()
{

}

void BattleEffectP012::Update()
{
	effectTimer++;
	if (effectTimer == 1)
	{
		for (auto& pos : hitPositionList)for (auto& obj : enemyMapList)
		{
			...
			{
				obj->CauseByPlayer(sourceID, 12,SPMAX);
			}
		}
...
void BattleEffectP012::Draw()
{
	for (auto& pos : hitPositionList)
	{
		Vector2<double> tempPos = Vector2<double>(pos.x*Define::MASU_SIZE + Define::MASU_SIZE / 2, pos.y*Define::MASU_SIZE + Define::MASU_SIZE / 2);
		DrawRotaGraphF((float)tempPos.x - camera.x, (float)tempPos.y - camera.y, 1.0, 0.0, graph[18][(effectTimer / 4) % 8], TRUE);
	}
}