...
const double BattleEffectP011_1::speed = 8.0;
const int BattleEffectP011_1::existTime = 40;

BattleEffectP011_1::BattleEffectP011_1(PlayerMap* playerMap, list<BossMapPtr>& bossMapList, list<ObjectMapPtr>& objectMapList, list<BattleEffectPtr>& battleEffectList, list<EnemyMapPtr>& enemyMapList, int sourceID, Vector2<int> masuPosition, Direction direction, Vector2<double>&camera, int& mapNum,bool SPMAX) :BattleEffect(playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, ObjectType::PLAYER, sourceID, masuPosition, direction, camera, mapNum, TargetType::ENEMY),SPMAX(SPMAX)
{
...
BattleEffectP011_1::~BattleEffectP011_1()
{

}

void BattleEffectP011_1::Update()
{
	position += velocity;
	...
	for (auto& obj : enemyMapList)
	{
		if (abs(obj->GetPosition().x - position.x) < ((double)Define::MASU_SIZE - 2.0) && abs(obj->GetPosition().y - position.y) < ((double)Define::MASU_SIZE - 2.0))
		{
			...
			masuPosition = next;
			battleEffectList.push_back(make_shared<BattleEffectP011_2>(playerMap, bossMapList, objectMapList, battleEffectList, enemyMapList, sourceID, masuPosition, direction, camera, mapNum,SPMAX));
			disappearFlag = true;
		}
	}
