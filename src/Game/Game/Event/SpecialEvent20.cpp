...
void SpecialEvent20::UpdateEvent()
{
	...
	if (eventTimer == 370)
	{
		...
		messages.push_back("プルルウウウウウウウウウウウウ！");
...
void SpecialEvent20::Draw()
{
	Vector2<int> offset = Vector2<int>(24, 26);
	...
	for (int i = 0; i < lengthY + 20; i++)for (int j = 0; j < lengthX; j++)
	{
		...
		if (0 <= (pos.y) && (pos.y) < MapInfoList::GetMapInfoList()[59].heightNum && 0 <= (pos.x) && (pos.x) < MapInfoList::GetMapInfoList()[59].widthNum)
		{
			DrawGraphF((float)Define::MASU_SIZE*j - camera.x, (float)Define::MASU_SIZE*i - camera.y + shake.y, mapBackChip[MapInfoList::GetMapInfoList()[59].mapChip[pos.y][pos.x]], TRUE);
		}
	}
	...
	for (int i = 0; i < lengthY + 20; i++)for (int j = 0; j < lengthX; j++)
	{
		...
		if (0 <= (pos.y) && (pos.y) < MapInfoList::GetMapInfoList()[59].heightNum && 0 <= (pos.x) && (pos.x) < MapInfoList::GetMapInfoList()[59].widthNum)
		{
			DrawGraphF((float)Define::MASU_SIZE*j - camera.x, (float)Define::MASU_SIZE*i - camera.y + shake.y, mapObjectChip[MapInfoList::GetMapInfoList()[59].mapObject[0][pos.y][pos.x]], TRUE);
		}
	}
	...
	for (int i = 0; i < lengthY + 20; i++)for (int j = 0; j < lengthX; j++)
	{
		...
		if (0 <= (pos.y) && (pos.y) < MapInfoList::GetMapInfoList()[59].heightNum && 0 <= (pos.x) && (pos.x) < MapInfoList::GetMapInfoList()[59].widthNum)
		{
			DrawGraphF((float)Define::MASU_SIZE*j - camera.x, (float)Define::MASU_SIZE*i - camera.y + shake.y, mapObjectChip[MapInfoList::GetMapInfoList()[59].mapObject[1][pos.y][pos.x]], TRUE);
		}
	}
	...
	DrawRotaGraph(bossPosition.x - camera.x, bossPosition.y - camera.y + shake.y, 1.0, 0.0, bossGraph[5], TRUE);
